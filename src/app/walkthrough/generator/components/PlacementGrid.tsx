    ...
    return (
        <>
            <div className="py-12 md:py-0 w-full h-full flex flex-col md:flex-row justify-around md:justify-between items-center">
                ...
                {/* grid */}
                <div className="grid border-[1px] w-full xs:w-[300px] xs:h-[300px] sm:w-[400px] sm:h-[400px]" style={{gridTemplateColumns: `repeat(${gridSize}, 1fr)`}}>
                ...
                <div className="grid gap-y-2 md:gap-6 grid-cols-2 md:grid-cols-1 flex-col justify-start items-start gap-6 py-6">
                    ...
                    <div className="flex flex-col mx-auto my-auto gap-1 col-start-1 row-start-2 col-span-2">
                        <div className="flex items-center justify-between w-[200px]">
                            <label htmlFor="grid-slider"></label>
                            <input id="grid-slider" type="range" min="4" max="12" value={gridSize} step="1" onChange={(e) => setGridSize(Number(e.target.value))}  className="slider w-[170px] h-6 px-1 bg-gray-200 rounded-full appearance-none cursor-pointer" />
                            <span>{gridSize}</span> 
                        </div>
                    </div>
                    ...
                    <motion.button 
                        ...
                        whileTap={{ scale: 0.95 }} 
                        onClick={() => setPlacementType(1)} 
                        className="col-start-1 row-start-4 flex items-center p-3 w-max sm:w-[145px] mx-auto md:mx-0 justify-between border rounded-md shadow-sm gap-3 md:gap-6"
                    >
                    ...
                    <motion.button 
                        whileHover={{ scale: 1.05 }} 
                        whileTap={{ scale: 0.95 }} 
                        onClick={() => setPlacementType(2)} 
                        className="col-start-2 md:col-start-1 row-start-4 md:row-start-5 mx-auto md:mx-0 flex items-center p-3 w-max sm:w-[145px] justify-between border rounded-md shadow-sm gap-3 md:gap-6"
                    >
                    ...
                    <motion.button 
                        ...
                        whileTap={{ scale: 0.95 }} 
                        onClick={clearGrid} 
                        className="w-max mx-auto md:mx-0 text-center col-start-1 col-span-2 md:col-start-1 row-start-6 md:row-start-6 flex items-center py-2 px-6 justify-between border rounded-md shadow-sm gap-6"
                    >
                        <span>Clear</span>
                    </motion.button>
...
const Tile = ({ id, onClick, colour }: TileProps) => {
    ...
    return (
        <div 
            style={{  }}
            onClick={() => onClick(id)}
            className={`${colour} w-full aspect-square flex items-center justify-center border-[1px] cursor-pointer hover:bg-black/20`}>
        </div>
    );
}